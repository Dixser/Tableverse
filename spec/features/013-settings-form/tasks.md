...
- [x] 1. `packages/game-core/src/settingsValidation.ts` +
...
- [x] 2. `packages/server/src/rooms/roomService.ts` — add
      ...
      **Verify:** integration tests — happy path, `in_game` rejection,
      ...
      (spec.md AC5).
- [x] 3. `packages/server/src/rooms/roomRoutes.ts` — extend
      ...
      **Verify:** route integration tests — valid `gameSettings` (200 +
      ...
      AC6/AC7).
- [x] 4. `packages/client/src/api/roomApi.ts` — add a `gameSettings`
      parameter to the existing settings-update call.
      **Verify:** client typechecks.
- [x] 5. `packages/client/src/room/SettingsForm.tsx` (+
...
- [x] 6. `packages/client/src/room/RoomShell.tsx` — render
      ...
      **Verify:** `RoomShell.test.tsx` — new cases for "no settingsSchema
      ...
      fields."
- [x] 7. Run `test:unit`/`typecheck` across every workspace. Full
